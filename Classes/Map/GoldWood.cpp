...
void Setting2::onMouseScroll(cocos2d::Event* event) {
	...
	float scaleChange = scrollY > 0 ? 0.1f : -0.1f;																// 根据滚动值设置缩放变化值
	...
	mapParentNode->runAction(easeAction);																		// 运行动作
	...
	currentPos.x = std::max(currentPos.x, minX);																// 设置 x 坐标
	currentPos.y = std::max(currentPos.y, minY);																// 设置 y 坐标
