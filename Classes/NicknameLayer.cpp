...
bool NicknameLayer::init() {
    ...
    this->setPosition(Vec2(0, 250));                                                                            // Set popup position
    ...
    confirmButton->addClickEventListener([=](Ref* sender) {
        ...
        if (nickname.empty()) {
            ...
            // ÇÐ»»µ½ MiniMap ³¡¾°
            auto miniMapScene = MiniMap::createWithMap("smallmap/castle.tmx", false);
            Director::getInstance()->replaceScene(miniMapScene);
        }
        });
